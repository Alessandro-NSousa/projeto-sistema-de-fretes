...
class PagamentoResourceIT {
    ...
    public static Pagamento createEntity(EntityManager em) {
        Pagamento pagamento = new Pagamento().valor(DEFAULT_VALOR).parcela(DEFAULT_PARCELA).formaPagamento(DEFAULT_FORMA_PAGAMENTO);
        return pagamento;
    }
    ...
    public static Pagamento createUpdatedEntity(EntityManager em) {
        Pagamento pagamento = new Pagamento().valor(UPDATED_VALOR).parcela(UPDATED_PARCELA).formaPagamento(UPDATED_FORMA_PAGAMENTO);
        return pagamento;
    }
    ...
    void createPagamento() throws Exception {
        ...
        assertThat(testPagamento.getParcela()).isEqualTo(DEFAULT_PARCELA);
    ...
    void getAllPagamentos() throws Exception {
        ...
        restPagamentoMockMvc
            ...
            .andExpect(jsonPath("$.[*].formaPagamento").value(hasItem(DEFAULT_FORMA_PAGAMENTO)));
    }

    @SuppressWarnings({ "unchecked" })
    void getAllPagamentosWithEagerRelationshipsIsEnabled() throws Exception {
        when(pagamentoRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPagamentoMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(pagamentoRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getPagamento() throws Exception {
        ...
        restPagamentoMockMvc
            ...
            .andExpect(jsonPath("$.formaPagamento").value(DEFAULT_FORMA_PAGAMENTO));
    }
    ...
    void putNewPagamento() throws Exception {
        ...
        updatedPagamento.valor(UPDATED_VALOR).parcela(UPDATED_PARCELA).formaPagamento(UPDATED_FORMA_PAGAMENTO);
        ...
        assertThat(testPagamento.getParcela()).isEqualTo(UPDATED_PARCELA);
    ...
    void partialUpdatePagamentoWithPatch() throws Exception {
        ...
        partialUpdatedPagamento.valor(UPDATED_VALOR).parcela(UPDATED_PARCELA).formaPagamento(UPDATED_FORMA_PAGAMENTO);
